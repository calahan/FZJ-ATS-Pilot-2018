...
author:
- Dean Calahan^[Smithsonian Institution, Forschungszentrum Jülich (visiting)]
...
logger was initialized and used to replace the previously active logger after harvest.
...
The data from each `csv` file was cleaned by plotting and visual inspection to determine
which observations referred to the logger's residence time within the floway. Results
of this analysis are available in the file `FZ-J WWTP ATS Pilot Data Cleaning`. In general it was obvious
...
### Total Nitrogen  
Deviation from the sawtooth pattern
...
# Acknowledgements  
The authors would like to thank Christina Kuchendorf, Marian Roeb, and Philipp Norf
for invaluable assistance during this project.
...
Figure 1.  Pilot ATS floway at Forshungszentrum-Jülich's wastewater treatment plant. (A) View
looking upslope towards headworks, with HOBO datalogger (inset) in foreground. (B) Side view
of installation, with reservoir tank on the left.

```{r Figure2}
# We need an ordered list of all the dates, not just the ones in the data frame
dates <- seq(min(wqb_tb$obsdate), max(wqb_tb$obsdate), by = 1)

# Plot parameters
oldpar <- par(no.readonly=T)

# Create the device for the abstract
png(fig_fn[2], width=6.5, height=3, units="in", res=600)

# Assign parameters for the plot
...
plot(wqb_tb$obsdate,
     ...
     type="l",
     xlab="Date",
     ylab=expression(paste("Total N (mg L"^"-1", ")")),
     cex.lab=0.8,
     axes=FALSE)

axis(side=1,
     labels=dates[c(1, 8, 15, 22, 29, 36, 43, 48)],
     at=dates[c(1, 8, 15, 22, 29, 36, 43, 48)],
     las=1,
     cex.axis = 0.6)
...
points(wqb_tb[wqb_tb$before==TRUE,]$obsdate, wqb_tb[wqb_tb$before==TRUE,]$TN, pch=1, cex=0.5)
points(wqb_tb[wqb_tb$before==FALSE,]$obsdate+0, wqb_tb[wqb_tb$before==FALSE,]$TN, pch=0, cex=0.5)
...
plot(wqb_tb$obsdate,
     ...
     type="l",
     xlab="Date",
     ylab=expression(paste("PO" ["4"], " Phosphorus (mg L"^"-1", ")")),
     cex.lab=0.8,
     axes=FALSE)
axis(side=1,
     labels=dates[c(1, 8, 15, 22, 29, 36, 43, 48)],
...
points(wqb_tb[wqb_tb$before==TRUE,]$obsdate, wqb_tb[wqb_tb$before==TRUE,]$PO4P, pch=1, cex=0.5)
points(wqb_tb[wqb_tb$before==FALSE,]$obsdate+0, wqb_tb[wqb_tb$before==FALSE,]$PO4P, pch=0, cex=0.5)
