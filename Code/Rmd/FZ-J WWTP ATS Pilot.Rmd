...
Two HOBO data loggers, ATS_1 and ATS_2, (Onset Computer Corporation, Bourne, MA, USA),
were used to gather temperature in degrees Celsius and illumination in lux, with
one observation recorded per minute. Each week one logger was installed in the floway,
while the other logger was idle. Prior to biomass harvest, the idle logger was initialized
and was then used to replace the active logger after harvest. Data was downloaded
and processed using HOBOware 3.7.15 (Onset Computer Corporation, Bourne, MA, USA),
to export csv files containing the temperature and illumination readings.
temperature and illumination data. The csv files were analyzed in R 3.4.2 using RStudio 1.0.143.

The data from each HOBO csv file was cleaned by plotting and visual inspection to determine which observations
reflected the residence time within the floway. The results of this analysis are shown in Figures 1-7. Each
weekly plot is followed by magnifications of each cutoff region. In general it was obvious from temperature and/or illumination readings which were the bracketing readings. During the data cleaning process, readings
were accumulated into a single data frame and exported to the file `FZJATSPilot.csv`.
...
before and after water exchange. Starting 
